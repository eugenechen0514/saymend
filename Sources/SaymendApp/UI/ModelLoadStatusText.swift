...
enum ModelLoadStatusText {
    ...
    ///
    /// 2026-08-25 補充：large-v3_turbo 連續三趟（首次／卸載後／重啟 App 後）分別是
    /// 632.65s、693.96s、670.94s——**每一次載入都是完整重編**（見 #23）。這讓 30 分鐘
    /// 這個地板暫時仍然安全（最慢的一趟也才 11.6 分鐘），但它的理由已經不是
    /// 「冷載入很罕見」，而是「冷載入是常態，而 30 分鐘連常態的兩倍都還沒到」。
