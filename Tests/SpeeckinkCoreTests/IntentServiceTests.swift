...
@Test func newContentWithEmptySessionUsesPolishTimeout() async {
    ...
    let out = await makeService(p).process(utteranceRaw: "呃你好", sessionText: "")
...
@Test func nonEmptySessionUsesEditTimeoutAndCarriesSession() async {
    ...
    _ = await makeService(p).process(utteranceRaw: "續句", sessionText: "首句。")
...
@Test func editCommandReturnsEditedSession() async {
    ...
    let out = await makeService(p).process(utteranceRaw: "欸星期二改成星期三", sessionText: "我們星期二開會。")
...
@Test func editCommandWithEmptySessionDegrades() async {
    ...
    let out = await makeService(p).process(utteranceRaw: "x", sessionText: "")
...
@Test func undoIntentMapsToUndo() async {
    ...
    let out = await makeService(p).process(utteranceRaw: "復原上一步", sessionText: "有內容")
    #expect(out == .undo)
}
...
@Test func unknownIntentFallsBackToNewContent() async {
    ...
    let out = await makeService(p).process(utteranceRaw: "嗨", sessionText: "前文")
    #expect(out == .newContent("嗨"))
}

@Test func providerFailureAndGarbageDegrade() async {
    let f = await makeService(ScriptedIntentProvider(.fail)).process(utteranceRaw: "x", sessionText: "")
    guard case .degraded = f else { Issue.record("應降級"); return }
    let g = await makeService(ScriptedIntentProvider(.reply("not json"))).process(utteranceRaw: "x", sessionText: "")
...
@Test func traditionalizeGuardAppliesToBothTextOutcomes() async throws {
    ...
    let o1 = await makeService(p1, language: .zhTW, traditionalize: try TraditionalizeGuard()).process(utteranceRaw: "x", sessionText: "")
    #expect(o1 == .newContent("乾淨"))
    let p2 = ScriptedIntentProvider(.reply(#"{"intent":"edit_command","text":"干净的字"}"#))
    let o2 = await makeService(p2, language: .zhTW, traditionalize: try TraditionalizeGuard()).process(utteranceRaw: "改", sessionText: "髒的字")
